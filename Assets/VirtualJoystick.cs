...
using UnityEngine;
using UnityEngine.EventSystems; // Получить доступ к интерфейсам Event
using UnityEngine.UI; // Доступ к пользовательским интерфейсам
...
public class VirtualJoystick : MonoBehaviour, IBeginDragHandler, IDragHandler, IEndDragHandler
{
    ...
    // Местоположение пальца и джойстика,
    // когда происходит перемещение
    private Vector2 originalPosition;
    private Vector2 originalThumbPosition;

    // Расстояние, на которое сместился пелец
    // относительно исходного местоположения
    ...
    void Start()
    {
        ...
        // Выключить площадку, сделав её невидимой
    ...
    public void OnBeginDrag(PointerEventData eventData)
    {
        ...
        // Зафиксировать мировые координаты,
        //откуда начато перемещение
        Vector3 worldPoint = new Vector3();
        RectTransformUtility.ScreenPointToWorldPointInRectangle(
            this.transform as RectTransform, eventData.position,
        ...
        // Поместить плошадку в исходную позицию
    ...
    {
        // Определить текущие мировые координаты
        // точки контакта пальца с экраном
        Vector3 worldPoint = new Vector3();
        RectTransformUtility.ScreenPointToWorldPointInRectangle(
            this.transform as RectTransform, eventData.position,
            eventData.enterEventCamera, out worldPoint);
