...
public class Indicator : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        // в Update(), если цель target будет назначена
        distanceLabel.enabled = false;

        // На запуске дождаться блтжайшего кадра перед отображением
    ...
    void Update()
    {
        // Цель исчезла? Если да, то индикатор надо убрать
        if(target == null)
        {
            Destroy(gameObject);
            return;
        }
        // Если цель присутствует, вычислить расстояние до неё
        ...
        if(showDistanceTo != null)
        {
            ...
            var distance = (int)Vector3.Magnitude(showDistanceTo.position
                - target.position);
        ...
        RectTransformUtility.ScreenPointToLocalPointInRectangle(
            transform.parent.GetComponent<RectTransform>(), screenPoint, Camera.main,
            out localPosition);
